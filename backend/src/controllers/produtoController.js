...
const atualizarProduto = async (req, res) => {
  try {
    ...
    const {
      codigo_produto, descricao, nome_categoria, nome_marca, nome_modelo,
      estoque_minimo, estoque_ideal, estoque_atual,
      preco_compra, preco_custo, preco_venda, margem_lucro,
    } = req.body;
    ...
    if (codigo_produto !== undefined && codigo_produto.trim() !== produtoExistente.codigo_produto) {
      return res.status(400).json({ erro: "O Código do Produto é imutável e não pode ser alterado." });
    }
    ...
    // Numéricos
    if (estoque_atual !== undefined) updateData.estoque_atual = parseInt(estoque_atual);
    if (estoque_minimo !== undefined) updateData.estoque_minimo = estoque_minimo === null ? null : parseInt(estoque_minimo);
    if (estoque_ideal !== undefined) updateData.estoque_ideal = estoque_ideal === null ? null : parseInt(estoque_ideal);

    if (preco_venda !== undefined) updateData.preco_venda = parseFloat(preco_venda);
    if (preco_compra !== undefined) updateData.preco_compra = preco_compra === null ? null : parseFloat(preco_compra);
    if (preco_custo !== undefined) updateData.preco_custo = preco_custo === null ? null : parseFloat(preco_custo);
    if (margem_lucro !== undefined) updateData.margem_lucro = margem_lucro === null ? null : parseFloat(margem_lucro);

    // Resolvendo chaves estrangeiras
    // Categoria
    if (nome_categoria !== undefined) {
      const catLimpa = nome_categoria.trim().toUpperCase();
      let catDB = await prisma.categoria.findUnique({ where: { nome: catLimpa } });
      if (!catDB) catDB = await prisma.categoria.create({ data: { nome: catLimpa } });
    ...
    if (nome_marca !== undefined || nome_modelo !== undefined) {
      ...
      const marcaFinal = nome_marca !== undefined 
        ? nome_marca.trim().toUpperCase() 
        : (produtoExistente.modelo ? produtoExistente.modelo.marca.nome : null);
        
      const modeloFinal = nome_modelo !== undefined 
        ? nome_modelo.trim().toUpperCase() 
        : (produtoExistente.modelo ? produtoExistente.modelo.nome : null);

      if (marcaFinal && modeloFinal) {
        let marcaDB = await prisma.marca.findUnique({ where: { nome: marcaFinal } });
        if (!marcaDB) marcaDB = await prisma.marca.create({ data: { nome: marcaFinal } });

        let modeloDB = await prisma.modelo.findFirst({ where: { nome: modeloFinal, fk_marca_id: marcaDB.id_marca } });
        if (!modeloDB) {
          modeloDB = await prisma.modelo.create({ data: { nome: modeloFinal, fk_marca_id: marcaDB.id_marca } });
        }
    ...
    if (error.code === "P2025") {
      return res.status(404).json({ erro: "Produto não encontrado na base de dados para atualização." });
    }
    
    return res.status(500).json({ erro: "Erro interno no servidor ao atualizar produto." });
  }
};
