            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 auto-rows-fr">
              {produtosFiltrados.map((p) => (
                <div
                  ...
                  onClick={() =>
                    setMenuAberto(
                      menuAberto === p.id_produto ? null : p.id_produto,
                    )
                  }
                >
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    {[p.modelo?.marca?.nome, p.categoria?.nome, p.modelo?.nome]
                      .filter(Boolean)
                      .join(" • ")}
                  </p>
                  <div className="flex items-center justify-between mt-3">
                    <span className="text-sm font-bold text-blue-600">
                      {formatarPreco(
                        p.preco_venda || p.preco_compra || p.preco_custo || 0,
                      )}
                    </span>
                  ...
                  {menuAberto === p.id_produto && (
                    ...
                    >
                      <Link
                        href={`/catalogo/produto/visualizar/${p.id_produto}`}
                      >
                        <button className="flex items-center gap-2 w-full px-4 py-2.5 text-sm text-green-500 hover:bg-green-50 transition-colors">
                          <Eye className="w-4 h-4" /> Visualizar
                        </button>
                      </Link>
                      <Link
                        href={`/catalogo/produto/atualizar/${p.id_produto}`}
                      >
